...
const VideoPlayer = () => {
  ...
  const videoData = [
    ...
    { title: "How does It generate money :car:", source: uberfeaturesVideo },
    { title: "Pros and Cons of Uber", source: userExperienceVideo },
    { title: "User Experience Of Uber", source: userExperienceVideo },
  ];
  return (
        ...
        <div className="topVideos">
          {videoData.slice(0, 3).map((video, index) => (
            <div
              key={index}
              className={`videoCard ${
                fullscreenVideo === index ? "fullscreen" : ""
              }`}
            >
              <video
                controls
        ...
        <div className="bottomVideos">
          {videoData.slice(3).map((video, index) => (
            <div
              key={index + 3}
              className={`videoCard ${
                fullscreenVideo === index + 3 ? "fullscreen" : ""
              }`}
            >
              <video
                controls
